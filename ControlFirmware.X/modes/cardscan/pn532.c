...
void pn532_service(void) {  
    ...
    switch(mode_data.cardscan.pn532_state) {
        ...
        case PN532_STATE_CONFIG:
            ...
            pn532_cmd_send(&mode_data.cardscan.spi_buffer[0], size, 9, &pn532_service_samconfigure_callback);         
            break;
        ...
        case PN532_STATE_DETECT_START:
            ...
            pn532_cmd_send(&mode_data.cardscan.spi_buffer[0], size, 0, &pn532_service_inpassivetarget_callback);  
            break;
        ...
        case PN532_STATE_DETECT_WAIT:
            if (!kpin_read(KPIN_C4)) {
                ...
                pn532_cmd_send(&mode_data.cardscan.spi_buffer[0], 0, 22, &pn532_service_detect_callback);  
            }
            break;
        ...
        case PN532_STATE_BLOCK_READ:
            ...
            pn532_cmd_send(&mode_data.cardscan.spi_buffer[0], size, 26, &pn532_service_mfu_read_callback);              
            break;
        ...
        case PN532_STATE_BLOCK_WRITE:
            writebuf[3] = mode_data.cardscan.programming_id;
            ...
            pn532_cmd_send(&mode_data.cardscan.spi_buffer[0], size, 10, &pn532_service_mfu_write_callback); 
            break;
        ...
        if (mode_data.cardscan.spi_buffer[5] == 0xd5 && mode_data.cardscan.spi_buffer[6] == 0x4b && mode_data.cardscan.spi_buffer[7] == 0x01) {
            ...
            if (atqa == 0x0044 && sak == 0x00 && uid_len == CARDSCAN_MFU_UID_LEN) {                   
                ...
                for (uint8_t i = 0; i < CARDSCAN_MFU_UID_LEN; i++) {
                    if (mode_data.cardscan.spi_buffer[13+i] != mode_data.cardscan.rfid_scan_uid[i]) {
                        different_card = true;
                    }
                    
                    mode_data.cardscan.rfid_scan_uid[i] = mode_data.cardscan.spi_buffer[13+i];
                }
                
                if (different_card || (mode_data.cardscan.rfid_scan_tick + CARDSCAN_SAME_CARD_COOLDOWN < tick_value)) {                    
                    if (mode_data.cardscan.programming) {
                        mode_data.cardscan.pn532_state = PN532_STATE_BLOCK_WRITE;
                    } else {
                        mode_data.cardscan.pn532_state = PN532_STATE_BLOCK_READ;
                    }
                    
                    mode_data.cardscan.rfid_scan_tick = tick_value;
                } else {
    ...
    if (ok) {                      
        if (mode_data.cardscan.spi_buffer[5] == 0xd5 && mode_data.cardscan.spi_buffer[6] == 0x41) {
            if (mode_data.cardscan.spi_buffer[7] == 0x00 &&
                ...
                // Read Success
                mode_data.cardscan.card_id = mode_data.cardscan.spi_buffer[10];
                mode_data.cardscan.card_new = true;
    ...
    if (ok) {                      
        if (mode_data.cardscan.spi_buffer[5] == 0xd5 && mode_data.cardscan.spi_buffer[6] == 0x41) {
            if (mode_data.cardscan.spi_buffer[7] == 0x00 && (mode_data.cardscan.spi_buffer[8] & 0x0f) == 0x0a) {
                // Write Success!
                mode_data.cardscan.programming_id++;
                
                if (mode_data.cardscan.programming_id >= CARDSCAN_POOL_SIZE) {
                    mode_data.cardscan.programming = 0;
                }
