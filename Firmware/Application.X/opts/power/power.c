...
i2c_command_t *power_i2c_callback(i2c_command_t *cmd) {
    ...
    switch (opt->power.poll_state) {
        case 1:
            ...
            // 0x00 0x3f (EN_HIZ=0 EN_LIM=0 IIN_LIM=3250mA)

            opt->power.poll_state++;
            opt->power.buffer[0] = 0x00;
            opt->power.buffer[1] = 0x3f;
            return cmd;

        case 2:
            // Configure system reset mode
            // 0x0a 0x40 (SW_FREQ=1.35Mhz, TIMR2X_EN=1, BATFET_DIS=0, SYSREST_SEL=0, TDISC_H=2s TDISK_L=8s)
